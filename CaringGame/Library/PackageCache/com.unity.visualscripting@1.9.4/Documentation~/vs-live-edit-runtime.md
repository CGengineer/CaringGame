...
> [!NOTE]
> If youfve used an embed graph and do not want to lose your modifications, copy all the changes you made to the embed graph before exiting Play mode. Paste them back in when in edit mode. You canft do this for any changed variables.
...
- In the Inspector, click in any field to change a componentfs value. The values are not persistent and wonft save when you leave Play mode.
...
> [!NOTE]
> When you modify anything after a Start event during runtime you wonft get the update on the GameObject during that session. You need to restart the session for Unity to execute the new logic after the start event.
